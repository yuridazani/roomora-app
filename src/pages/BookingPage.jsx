...
const BookingPage = ({ navigateTo, context }) => {
  ...
  const { showModal } = useModal(); // Gunakan hook modal
  ...
  const handleProceedToPayment = () => {
    if (!guestDetails.fullName || !guestDetails.email) {
      ...
      showModal(
        ...
        'Please fill in your full name and email address to proceed.'
      );
      return;
    }
    // Gabungkan semua data dan teruskan ke halaman sukses
    navigateTo('success', {
      ...context,
      ...guestDetails,
      nights,
  ...
  return (
    <div className="min-h-screen bg-midnight-green flex items-center justify-center p-6">
      <div className="container max-w-4xl grid md:grid-cols-2 gap-8 bg-champagne rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-8">
          <h1 className="font-clash text-3xl font-bold text-midnight-green">
            Confirm Your Booking
          </h1>
          <p className="text-charcoal/80 mt-2">
            You're one step away from your dream stay.
          </p>
          ...
          <div className="mt-8">
            ...
            <div className="flex justify-between items-center mt-4">
              <div>
                ...
                <h2 className="font-clash text-xl font-semibold text-midnight-green">
                  {property.name}
                </h2>
              </div>
              <p className="font-bold text-lg">
                Rp {property.price.toLocaleString('id-ID')}/night
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 border-t border-rose-taupe/30 pt-6 space-y-4">
            <div className="flex items-center">
              <Calendar className="text-rose-taupe mr-3" />
              <span>
                {context.checkIn} to {context.checkOut} ({nights} nights)
              </span>
            </div>
            <div className="flex items-center">
              <Users className="text-rose-taupe mr-3" />
              <span>{context.guests} Guests</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 bg-white/50">
          <h2 className="font-clash text-2xl font-semibold text-midnight-green">
            Guest Details
          </h2>
          <div className="mt-4 space-y-4">
            <input
              ...
              className="w-full p-3 rounded-lg border focus:ring-bright-pink focus:border-bright-pink"
            />
            <input
              type="email"
          ...
          <h2 className="font-clash text-2xl font-semibold text-midnight-green mt-8">
            Payment Method
          </h2>
          ...
          <div className="mt-8 border-t border-rose-taupe/30 pt-4">
            <div className="flex justify-between text-charcoal/80">
              <span>
                {nights} nights x Rp {property.price.toLocaleString('id-ID')}
              </span>
            ...
            <div className="flex justify-between font-bold text-xl mt-2 text-midnight-green">
              <span>Total</span>
              <span>Rp {totalPrice.toLocaleString('id-ID')}</span>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full bg-bright-pink text-white font-bold py-3 rounded-full mt-6 hover:bg-opacity-90 transition-all transform hover:scale-105"
          >
            Confirm & Pay
          </button>
        </div>
